...
export default {
  ...
  computed: {
    ...
    table () {
      return this.tableData.map(table => {
        ...
        return Object.assign(table, { dialogFlag: 1 }) // 在元数据中添加一个弹窗数据标识，用于展示删除异常操作
      })
    },
    renderColumns () {
  ...
  mounted () {
    ...
    this.$bus.$on('outpatient:delete-exception-cause', (row, index) => {
  ...
  methods: {
    ...
    abnormallyDeleteHandler (row, index, type) {
      ...
      this.table[index]['identificationInfo'] = ''
      this.columnsNum++

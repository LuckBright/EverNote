...
export default {
  ...
  methods: {
    ...
    list (isUpdate) {
      // 库房配置信息读取未添加字段
      const { id: processConfigurationId, storageRoomId, scene } = this.processConfig
      const params = {
        pagesize: this.pagesize,
        offset: this.offset,
        typeList: JSON.parse(scene),
        examineFlag: 2,
    ...
    handlerRowClick ({ row, column }) {
      const { visitSn: visitNumber, patientId, identification, channel, scene } = row
      const index = this.tableData.findIndex(item => item.visitSn === row.visitSn)

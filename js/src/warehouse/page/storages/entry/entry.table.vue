    ...
    <div class="table">
      ...
      <div>
        <el-table
          ...
          <el-table-column label="加成率%" width="100" align="center" v-if="isBatch">
            <template slot-scope="scope">
          ...
          <el-table-column label="最高限价" width="100" align="center" v-if="isBatch">
  ...
  methods: {
    ...
    handlerBatchBlur (row, index) { // 校验同一物资是否有相同批次
      ...
      if (batchList) {
        ...
        if (mater.length > 1) {
          row.batchNum = ''
          return this.$messageTips(this, 'warning', '该物资已有相同的生产批号，请重新输入')
        }
      }
    },
      ...
      this.list.item.forEach(item => {
        if (item.localMaterialId === data.localMaterialId) {
          ...
          if (this.isBatch) {
            ...
            // 如果原有数据中存在 priceList 的长度 则更新一遍里面加成率和基础价
    ...
    handleAddRow (data) {
      ...
      if (data.localMaterialId && data.isExpiryDate && (!data['batchNum'] || !data['produceDate'] || !data['validityDate'] || !data['num'] || data.price === undefined)) {
        return this.$messageTips(this, 'warning', '该商品生产批号、生产日期和有效期信息未完成，请完成后再添加')
      }

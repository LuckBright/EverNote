<template>
  <div class="service-query">
    ...
    <div v-if="noHasService">
      <el-input :value="service.name" clearable @input="change" class="no-service"></el-input>
      <!-- <span class="error-status">(已停用/无使用权)</span> -->
    </div>
  </div>
</template>
<script>
import api from '@/sob/store/service.merge'
...
export default {
  ...
  methods: {
    async remoteMethod (query) {
      this.loading = true
      let res = await api.getTenantServiceAndSet({
        name: query,

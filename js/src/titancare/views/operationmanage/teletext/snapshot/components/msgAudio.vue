<template>
  ...
  <div v-if="msgData.msgFrom === MSGFROM[2]" class="chat chat-right">
    <p class="text audio" :style="'width: ' + ((msgData.msgContent.second / 30) * 100 + 40) + 'px'" @click='playAudio(msgData)'>
...
<script>
import { MSGFROM, URL } from '../common/config'
import IMGMAP from '../assets/images'
import Bus from '../common/mq.bus'
...
export default {
  ...
  destroyed () {
    Bus.$emit('mqStop', this.msgData.msgContent.url)
    this.currentRandom = ''

<template>
  <div class="layout_inner">
    ...
    <el-table
      ...
      <el-table-column label="视频问诊服务" prop="videoStatus" width="120">
        <template slot-scope="scope">
          <span>{{scope.row.videoStatus === 1 ? '已开通' : '未开通'}}</span>
        </template>
      </el-table-column>
      <el-table-column label="图文问诊服务" prop="itemStatus" width="120">
        <template slot-scope="scope">
          <span>{{scope.row.itemStatus === 1 ? '已开通' : '未开通'}}</span>
        </template>
      </el-table-column>
      <el-table-column label="默认服务价格(元/次)" prop="videoStatus" width="120">
        <template slot-scope="scope">
          <span>{{scope.row.price}}</span>
        </template>
      </el-table-column>
      <el-table-column label="图文问诊允许医生定价" prop="itemAllowCustom" width="120">
        <template slot-scope="scope">
          <span>{{scope.row.itemAllowCustom === 1 ? '是' : '否'}}</span>
        </template>
      </el-table-column>
      <el-table-column label="在线咨询服务" prop="consultStatus" width="120">
        <template slot-scope="scope">
          <span>{{scope.row.consultStatus === 1 ? '已开通' : '未开通'}}</span>
        </template>
      </el-table-column>
      <el-table-column label="在线咨询允许医生定价" prop="consultAllowCustom" width="120">
        <template slot-scope="scope">
          <span>{{scope.row.consultAllowCustom === 1 ? '是' : '否'}}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog
      ...
      <template slot="footer">
        ...
        <el-button type="primary" @click="addInfoClick">保存</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
      ...
      api.getServiceItem({
        ...
        if (res.errCode === 0) {
          const commonData = {
            ...
            tenantName: '', // 集团名称
            title: row.skillsTitle2Name, // 医生职称
            titleCode: row.skillsTitle2Code, // 医生编码
          ...
          const commonOnlineData = {
            serviceName: '在线咨询',
            serviceTypeCode: '8',
          }
          const imgList = res.data.filter(item => item.serviceTypeCode === '1')
          const videoList = res.data.filter(item => item.serviceTypeCode === '4')
          const onlineList = res.data.filter(item => item.serviceTypeCode === '8')
          this.dialogData = [
     ...
     */
    addInfoClick () {
      const imgData = { ...this.$refs.dialogRef.imgData }
      const videoData = { ...this.$refs.dialogRef.videoData }
      const onlineData = { ...this.$refs.dialogRef.onlineData }
      let params = Object.assign({}, this.$refs.dialogRef.data)
      let reg = /((^[1-9]\d*)|^0)((\.\d{0,2}){0,1})$/

      // if (imgData.status) {
      // }
      if (!params.orgId || !params.departmentCode) {
        this.$notify({
          title: '警告',
          message: '请完善信息，必填(所在机构,所在科室,图文问诊默认服务)',
          type: 'warning'
        })
        return false
      }
      ...
      if (imgData.status) {
        ...
        if (priceInfo && priceInfo !== 'null') {
          ...
          this.$notify({
            title: '警告',
            message: '[图文问诊服务]请输入自定义价格',
            type: 'warning'
          })
          return false
        }
        if (!imgData.serviceId) {
          this.$notify({
            title: '警告',
            message: '[图文问诊服务]请选择默认服务',
            type: 'warning'
          })
          return false
        }
      ...
      if (onlineData.status) {
        let priceInfo = onlineData.price + ''
        if (priceInfo && priceInfo !== 'null') {
          let priceValue = priceInfo.search(reg)
          if (priceValue === -1 || priceInfo > 9999) {
            this.$notify({
              title: '警告',
              message: '[在线咨询服务]请输入正确自定义价格（保留两位小数），且不能大于9999',
              type: 'warning'
            })
            return false
          }
          ...
          this.$notify({
            title: '警告',
            message: '[在线咨询服务]请输入自定义价格',
            type: 'warning'
          })
          return false
        }
        if (!onlineData.serviceId) {
          this.$notify({
            title: '警告',
            message: '[在线咨询服务]请选择默认服务',
            type: 'warning'
          })
          return false
        }
      }
      params.allowCustom = imgData.allowCustom ? 1 : 2
      params.status = imgData.status ? 1 : 0
      params.allowCustomPatient = imgData.allowCustomPatient ? 1 : 2
      params.online = params.online ? 1 : 2
      params.serviceId = imgData.serviceId
      params.price = imgData.price
      if (imgData.allowCustom) {
        params.customPriceMax = imgData.customPriceMax
      }

      if (params.online === 1 && params.status && params.allowCustom === 1) {
        ...
        if (price && price !== 'null') {
          ...
          if (isTrue === -1 || price > 9999) {
            this.$notify({
              title: '警告',
              message: '[图文问诊服务]请输入正确价格（保留两位小数），且不能大于9999',
              type: 'warning'
            })
            return false
          }
          ...
          this.$notify({
            title: '警告',
            message: '[图文问诊服务]请输入价格上限',
            type: 'warning'
          })
          return false
        }
      }
      if (params.online === 1 && onlineData.status && onlineData.allowCustom) {
        let price = onlineData.customPriceMax + ''
        if (price && price !== 'null') {
          let isTrue = price.search(reg)
          if (isTrue === -1 || price > 9999) {
            this.$notify({
              title: '警告',
              message: '[在线咨询服务]请输入正确价格（保留两位小数），且不能大于9999',
              type: 'warning'
            })
            return false
          }
      ...
      const commonData = {
        ...
        allowCustomPatient: params.allowCustomPatient,
      }
      const paramsImgData = {
        ...imgData,
        ...commonData,
      }
      const paramsVideoData = videoData.serviceTypeCode ? {
        ...videoData,
        ...commonData,
        status: videoData.status ? 1 : 0,
        itemCode: null,
        itemName: null,
        serviceName: '视频问诊',
      } : {}
      const paramsOnlineData = onlineData.serviceTypeCode ? {
        ...onlineData,
        ...commonData,
        status: onlineData.status ? 1 : 0,
        allowCustom: onlineData.allowCustom ? 1 : 2,
        allowCustomPatient: imgData.allowCustomPatient ? 1 : 2,
        serviceId: onlineData.serviceId,
        price: onlineData.price,
        customPriceMax: onlineData.customPriceMax,
      } : {}
      // console.log(params, 'params')
      // console.log(paramsImgData, 'paramsImgData')
      // console.log(paramsVideoData, 'paramsVideoData')
      this.addSend(paramsImgData, paramsVideoData, paramsOnlineData)
    },
    /**
     * 保存发送
     */
    addSend (params, videoData, onlineData) {
      // if (this.isAllSend) {
      //   let list = []
      ...
      api.creatServiceItem([params, videoData, onlineData]).then(res => {
        if (res.errCode === 0) {
          this.$message({
            message: '保存成功',
            type: 'success'
    ...
    查看资料 */
    seeInfoClick (val) {
      window.open(this.$ever.managesIndex + '/manages/org/staff/detail/' + val.id + '/edit?isView=1')
    },
    /**
     * 查询是否有未关闭的图文问诊
     * */

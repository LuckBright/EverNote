...
const url = diction.api + '/rcm/'
import { baseHost } from '@/store/basehost.js'
...
rcm.interceptors.request.use(function (config) {
  ...
  try {
    host = JSON.parse(localStorage.getItem('CLINICCONFIG'))['insurance_localhost']
  } catch (err) {
    console.log(err)
  }
  config.headers['global-host'] = host
  config.headers['thc-host'] = baseHost.host
  return config
}, function (error) {

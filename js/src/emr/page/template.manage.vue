<template>
  <div class="template_manage flex_column_full_hidden" :class="{'w_auto': showTpl}">
    <div class="sidebar_box flex_column_full_hidden row" :style="{width: showSelect ? '310px' : '30px'}">
      <nav-bar
        ref="template_nav"
        :workshop="workshop"
        :role="role"
      ...
      <div class="right_menu flex_column_1_hidden"> 
        <div class="top_bar search">
          ...
          <el-checkbox-group v-model="checkList" :min="1" v-if="String(businessType)!=='2'">
            <el-checkbox :label="2" :key="2">已审核</el-checkbox>
            <el-checkbox :label="1" :key="1">待审核</el-checkbox>
          </el-checkbox-group>
        </div>
...
export default {
  ...
  data () {
    return {
      ...
      showSelect: true,  // 是否显示列表区域
      templateNameList: [], // 模板名称列表
      selectTemp: '', // 选中的模板名称
      checkList: [1, 2], // 是否审核，全选，全部，1，未审，2 已审
      reviewAble: true, // 是否有审核权限  true 有 false 没有
    }
  },
  computed: {
  ...
  methods: {
    async getSkillTitle () { // 获取用户职称
      ...
      if (res.head.errCode === 0) {
        ...
        this.reviewAble = toRole(this.role).indexOf('high') > -1
      }
    },
    getFormInfo () {
    ...
    getTemplateTree (deptIds, templateName) {
      ...
      if (this.checkList.length < 2) {
        params.auditStatus = this.checkList[0]
      }
      request(this.urlMap.templateManage.getTemplateTree, params).then(res => {
        this.treeData = res.data
        this.templateNameList = []
        this.templateList.forEach(v => {
          if (this.businessType === 3 && deptIds && deptIds.length) {
            if (this.treeData[deptIds[0]][v.id] && this.treeData[deptIds[0]][v.id].length) {
              this.getTemplateNameList(this.treeData[deptIds[0]][v.id], v.id)
            }
          } else {
            if (this.treeData[v.id] && this.treeData[v.id].length) {
              this.getTemplateNameList(this.treeData[v.id], v.id)
            }
          }
        })
      })
    },
    getTemplateNameList (data, collapseId) {
      data.forEach(item => {
        if (item.isTemplate) {
          this.templateNameList.push(Object.assign({}, item, {collapseId: collapseId}))
          // 解决保存或审核后更新tree  当前节点未高亮问题
          if (this.$route.query.templateId === item.formTemplateId) {
            this.$nextTick(() => {
              this.$refs.templatetree.$refs['tree' + collapseId][0].setCurrentKey(Object.assign({}, item, {collapseId: collapseId}))
            })
          }
        }
        if (item.nodes && item.nodes.length) {
          this.getTemplateNameList(item.nodes, collapseId)
        }
      })
    },
    updateKey () {
    ...
    // 审核/撤销审核
    contextmenuReview (node) {
      let params = {
        id: node.id,
        auditStatus: Number(node.auditStatus) === 2 ? 1 : 2
      }
      let tips = Number(node.auditStatus) === 2 ? '撤销审核' : '审核'
      request(this.urlMap.templateManage.reviewTemplate, params).then(res => {
        if (res.head && res.head.errCode === 0 && res.data) {
          this.$messageTips(this, 'success', tips + '成功', '提示')
          this.renderTree(this.businessType)
          this.setAsyncEditor(resetEditorObj(res.data))
          this.updateKey()
        }
      })
    },

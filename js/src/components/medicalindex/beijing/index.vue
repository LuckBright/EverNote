...
export default {
  ...
  methods: {
    async getData () {
      ...
      try {
        const res = await api.getHomePage({ visitId: this.visitId })
    ...
    async add () {
      ...
      const res = await api.addHomePage(this.form)
    ...
    async echoData () {
      const res = await api.getEchoData({ visitId: this.visitId, patientId: this.patientId })

...
export default {
  ...
  methods: {
    ...
    change (val) {
      let value = val.split('&')
      this.$emit('change', {code: value[0], value: value[1]}, this.index)
    },
    async mixOptions () {
      ...
      level.map((item, _index) => {
        ...
        code.map(code => {
          const label = _label + '/' + code.name + '&' + item.code + '/' + code.code // 需要给后端的值
          const value = _label + '/' + code.name
          this.levelOprions.push({ label: label, value: `${index++}`, displayVal: value })
        })
      })
    }
  },
